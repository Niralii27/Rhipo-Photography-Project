...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM exclusive_clips WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Update query
    $stmt = $conn->prepare("UPDATE exclusive_clips SET title = ?, video = ?, status = ? WHERE id = ?");
    $stmt->bind_param("sssi", $title, $uploadedVideo, $status, $id);

    if ($stmt->execute()) {
        echo "<script>alert('Exclusive Clips updated successfully!');</script>";
        echo "<script>window.location.href = 'update_exclusive_clips.php?id=$id';</script>";
    } else {
