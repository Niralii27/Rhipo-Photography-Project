...
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM preferred_vendors WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Build update query
    $sql = "UPDATE preferred_vendors SET  title = ?, name = ?, description = ?, instagram = ?, facebook = ?";
    $params = [$title, $name, $description, $instagram, $facebook];
    $types = "sssss";
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>window.location.href = 'update_preferred_vendor.php?id=$id';</script>";
    } else {
...
<html>
<head>
    <title>Update Preferred Vendor</title>
...
<body>
<div class="container">
    <h2>Update Preferred Vendor</h2>
    <form id="reviewForm" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo $row['id']; ?>">

       
        <div class="form-group">
            <label for="title">Vendors Title</label>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="instagram" name="instagram" value="<?php echo htmlspecialchars($row['instagram'], ENT_QUOTES, 'UTF-8'); ?>" required>
        </div>

        <div class="form-group">
            <label for="name">Facebook</label>
            <input type="text" class="form-control" id="facebook" name="facebook" value="<?php echo htmlspecialchars($row['facebook'], ENT_QUOTES, 'UTF-8'); ?>" required>
        </div>
