<?php
	...
	include('config.php');
        ...
        $sql = "SELECT * FROM exclusive_clips";
        if (!empty($search_query)) {
            $sql .= " WHERE title LIKE '%$search_query%' OR status LIKE '%$search_query%'";
        }
        $sql .= " LIMIT $limit OFFSET $offset";
        $result = $conn->query($sql);
        
        // Get total number of records for pagination calculation
        $total_sql = "SELECT COUNT(*) as total FROM exclusive_clips";
        if (!empty($search_query)) {
            $total_sql .= " WHERE title LIKE '%$search_query%' OR status LIKE '%$search_query%'";
        }
...
<div class="main-panel">
				<div class="content">
					<div class="container-fluid">
						<h4 class="page-title">Exclusive Clips</h4>
						

						<div class="card">
									<div class="card-header">
										<div class="card-title"><button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addBlogModal">ADD CLIPS</button>

										</div><br>
                                        <form method="GET" action="" class="mb-3 d-flex">
                <input type="text" name="search" class="form-control me-2" placeholder="Search by title or status" value="<?php echo htmlspecialchars($search_query); ?>" style="max-width: 300px;">
                <button type="submit" class="btn btn-primary me-2" style="margin-right:10px;">Search</button>
                <a href="exclusive_clips.php" class="btn btn-secondary">Clear Search</a>
            </form>
                               ...
                               if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<td>
                                            <form method='POST' action='delete_exclusive_clips.php' onsubmit='return confirmDelete()'>
                        ...
                        <ul class="pagination justify-content-center">
                            ...
                            if ($page > 1) {
                                echo '<li class="page-item"><a class="page-link" href="?page=' . ($page - 1) . '&search=' . urlencode($search_query) . '">&laquo; Previous</a></li>';
                            }

                            // Display Page Numbers
                            for ($i = 1; $i <= $total_pages; $i++) {
                                echo '<li class="page-item ' . ($i == $page ? 'active' : '') . '"><a class="page-link" href="?page=' . $i . '&search=' . urlencode($search_query) . '">' . $i . '</a></li>';
                            }

                            // Display Next Page Link
                            if ($page < $total_pages) {
                                echo '<li class="page-item"><a class="page-link" href="?page=' . ($page + 1) . '&search=' . urlencode($search_query) . '">Next &raquo;</a></li>';
                            }
                            ?>
                        </ul>
                    </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addBlogModalLabel">Add Exclusive Clips</h5>
            ...
            <div class="modal-body">
                ...
                <iframe src="add_exclusive_clips.php" width="100%" height="800" style="border: none;"></iframe>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="updateBlogModal" tabindex="-1" aria-labelledby="updateBlogModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="updateBlogModalLabel">Update Exclusive Clips</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close" style="cursor: pointer;">
					<i class="la la-close"></i>
				</button>
            </div>
...
<script>
    ...
    window.location.href = 'exclusive_clips.php';  // Adjust the URL as needed
});
...
window.addEventListener('message', function(event) {
    ...
    if (message.action === 'success') {
        ...
        alert("New Exclusive Clips Entry created successfully!");
...
function loadUpdatePage(id) {
        ...
        iframe.src = `update_exclusive_clips.php?id=${id}`;
    }
    ...
    document.getElementById('updateBlogModal').addEventListener('hidden.bs.modal', function () {
        window.location.href = 'exclusive_clips.php'; // Reload to see updates
    });
    ...
    return confirm("Are you sure you want to delete this Exclusive Clips?");
}
